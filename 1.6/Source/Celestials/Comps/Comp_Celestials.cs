...
namespace Celestials
{
    public class Comp_Celestials : ThingComp
    {
        ...
        public override void PostSpawnSetup(bool respawningAfterLoad)
        {
            ...
            if (!ViableCandidate())
            {
                if ((parent as Pawn).health.hediffSet.HasHediff(CelestialsDefOf.Celestial_Hediff))
                {
        ...
        public void AddCelestialHediff(bool disableWarning = false)
        {
            if (isCelestial)
            {
                Hediff hediff = HediffMaker.MakeHediff(CelestialsDefOf.Celestial_Hediff, parent as Pawn);
        ...
        public void RemoveCelestialHediff()
        {
            if ((parent as Pawn).health.hediffSet.HasHediff(CelestialsDefOf.Celestial_Hediff))
            {
                Hediff hediff = (parent as Pawn).health.hediffSet.hediffs.Find(h => h.def == CelestialsDefOf.Celestial_Hediff);
        ...
        {
            if(!parent.def.IsLivingCreature() || parent.def.defName == "Zombie")
            {
                return false;
            }
            return true;
